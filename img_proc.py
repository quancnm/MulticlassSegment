...
def read_image_mask(x, y, IMG_SHAPE =[320, 416] ):
    ...
    x = cv2.resize(x, (IMG_SHAPE[0], IMG_SHAPE[1]))
    y = cv2.resize(y, (IMG_SHAPE[0], IMG_SHAPE[1]))
...
def preprocess_mask(mask_path, IMG_SHAPE):
    mask = cv2.imread(mask_path, cv2.IMREAD_COLOR)
    mask = cv2.resize(mask, (IMG_SHAPE[0], IMG_SHAPE[1]))
